...
const Login = () => {
  ...
  const { mutate: loginUser, data, isPending, isError, error } = useMutation({
    mutationFn: authUser 
  })
    ...
    <div className="flex justify-center items-center min-h-screen">
      ...
      <div className="m-2 p-4 flex flex-row justify-between gap-8 flex-initial">
        <div >
            ...
            <input className="input input-bordered w-full" placeholder="Email" name="email" value={auth.email} onChange={onChange}/>
        </div>
      ...
      <div className=" m-2 p-4 ">
        ...
        <>
          <button className="btn w-full bg-blue-600 text-gray-100 font-semibold hover:bg-blue-500 " onClick={handleSubmit}>Login</button>
        </>}
      </div>
      <div className="ml-6">
        Not a User? <a href="/auth/Register"className="link link-hover text-blue-600">Register</a> here.
      </div>
