...
const ListTemplate = ({items, titles, properties, infoPage }) => {
  ...
  return (
    <div className="flex m-2 p-4 flex-col w-full justify-items-stretch">
        <div className="flex flex-row p-4 mb-2 border-b-2 border-gray-600 gap-2">
            ...
            {titles.map((title: any) => {
                return (<div  className="flex-grow w-full font-semibold" key={title}>
                    {title}
                </div>) })}
        </div>
        ...
        <div className="p-2">
            ...
            {total > 10 && renderPagination(page, totalPages, handlePageClick)}
        </div>
